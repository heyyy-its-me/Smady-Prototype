...
export class N8nWebhookAdapter implements ExecutionAdapter {
  ...
  private getTimeoutMs(agentId: string): number {
    return agentId === 'leads' ? LEAD_MANAGEMENT_TIMEOUT_MS : this.timeoutMs;
  }
  ...
  async startExecution(request: StartExecutionRequest): Promise<ApiExecutionResponse> {
    ...
    try {
      ...
      // An acknowledgement means n8n will report completion through the
      // callback endpoints. Keep the corresponding UI execution running.
      if (isWorkflowStartedAcknowledgement(responseBody)) {
        return {
          ...
          nodes: [
            {
              id: `${request.agentId}-workflow`,
              name: `${request.agentId === 'leads' ? 'Lead Management' : 'Outreach'} workflow`,
              status: 'running',
              duration: 0,
              input: '',
              output: '',
